...
export const News = () => {
  ...
  return (
    <div className="space-y-10 py-8 relative">
      ...
      {/* Sticky Filter Bar */}
      <div className="sticky top-14 sm:top-16 z-30 bg-white border-y border-border py-4 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="relative w-full md:w-80">
            <input
              ...
              className="w-full pl-9 pr-4 py-2 border border-border rounded-button text-sm font-body focus:outline-none focus:ring-2 focus:ring-crimson focus:border-transparent transition-all duration-200"
            />
            <svg
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        {pinnedAnnouncements.length > 0 && (
          <section className="space-y-4">
            <div className="flex items-center space-x-2 select-none text-gold">
              ...
              <h2 className="font-display font-bold text-lg text-navy tracking-tight">Pinned Announcements</h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {pinnedAnnouncements.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="bg-white border-l-4 border-[#D4AF37] border-y border-r border-border rounded-r-card p-6 shadow-sm hover:shadow-subtle transition-all duration-300 flex flex-col justify-between"
                  >
                    <div>
                      ...
                      <h3 className="font-display font-bold text-navy text-base leading-snug hover:text-crimson cursor-pointer transition-colors mb-2" onClick={() => setSelectedNotice(item)}>
                        {item.title}
                      </h3>
                      <p className="text-textSecondary text-xs leading-relaxed font-body line-clamp-3 mb-4">
                        {item.body}
                      </p>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-1.5 select-none">
                        {item.tags.map((tag) => (
                          <span key={tag} className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-3xs font-body">
                            #{tag}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-border select-none">
                        <button
                          ...
                          className="text-xs font-semibold text-crimson hover:text-navy transition-colors flex items-center space-x-1"
                        >
        ...
        <section className="space-y-6">
          <h2 className="font-display font-bold text-lg text-navy tracking-tight select-none">Latest Updates</h2>
          ...
          {feedAnnouncements.length > 0 ? (
            ...
            pinnedAnnouncements.length === 0 && (
              <div className="text-center py-20 border border-dashed border-border rounded-card select-none">
                ...
                <h3 className="font-display font-bold text-navy text-base mb-1">No Notices Found</h3>
        ...
        {selectedNotice && (
          ...
          >
            <motion.div
              ...
              className="bg-white max-w-2xl w-full rounded-card overflow-hidden shadow-premium flex flex-col max-h-[85vh]"
            >
              {/* Modal Header */}
              <div className="p-6 border-b border-border flex justify-between items-start bg-slate-50 select-none">
                <div>
                  ...
                  <p className="text-3xs font-medium text-textSecondary mt-2">
                    Published on {formatDate(selectedNotice.publishedAt)}
                  </p>
                </div>
                <button
                  onClick={() => setSelectedNotice(null)}
                  className="text-slate-400 hover:text-navy transition-colors focus:outline-none"
              ...
              <div className="p-6 overflow-y-auto space-y-6">
                <h2 className="font-display font-extrabold text-navy text-lg sm:text-xl leading-tight">
                  {selectedNotice.title}
                </h2>
                
                {/* Description */}
                <p className="text-textSecondary text-sm leading-relaxed font-body whitespace-pre-line">
                  {selectedNotice.body}
                </p>

                {/* Attachments */}
                {selectedNotice.attachment && (
                  <div className="bg-surface border border-border p-4 rounded-card flex items-center justify-between select-none">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h4 className="text-xs font-bold text-navy">Official Reference Document</h4>
                ...
                {/* Tags */}
                <div className="flex flex-wrap gap-1.5 select-none pt-4 border-t border-border">
                  {selectedNotice.tags.map((tag) => (
                    <span key={tag} className="px-2.5 py-1 bg-slate-100 text-slate-600 rounded text-xs font-body">
                      #{tag}
                    </span>
                  ))}
                </div>
